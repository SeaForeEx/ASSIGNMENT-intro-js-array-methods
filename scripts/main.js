...
const buttonFilter = (event) => {
  ...
  if(event.target.id.includes('cartFilter')) {
    const uponAStar = referenceList.filter(enchilada => enchilada.inCart); //automatically returns true w/o '=== true' code
    renderCards(uponAStar);
  }
  if(event.target.id.includes('books')) {
    const deadTrees = referenceList.filter(quesadilla => quesadilla.type.toLowerCase() === 'book');
    renderCards(deadTrees);
  }
...
// .map()
const productList = () => {
  return referenceList.map(chimichanga => ({
    title: chimichanga.title,
    price: chimichanga.price,
    type: chimichanga.type
  }))
}
